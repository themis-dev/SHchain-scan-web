  ...
  <div class="bg">
    ...
    <div class="container flex-center">
      <div class="w-md-55 faucet-container">
        ...
        <div class="card-container">
          <span class="card-one" :class="{ 'active': isHTFaucet }" @click="switchToHT">DBOX</span>
          <span class="card-two" :class="{ 'active': !isHTFaucet }" @click="switchToErc">ARC-20</span>
        </div>
        ...
        <div class="verify-text" v-if="!isVerify">
          {{ $t('faucet.use') }}<a class="link-to" @click="dialogVisible = true">GitHub</a>{{ $t('faucet.verifyText') }}
        </div>
      </div>
    </div>
    <el-dialog
    ...
    >
      <div class="verify-container">
        <div class="oauth-box pointer" @click="toGithub">
          <img src="../../images/github.svg" alt="github">
        </div>
        <div class="verify-text">{{ $t('faucet.tips') }}</div>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import { getJWT, getContractList, faucet } from '@/api/chains'
  ...
  export default {
    ...
    computed: {
      disabled() {
        if (!this.$route.query.code && (this.isHTFaucet && this.address || !this.isHTFaucet && this.contractAddress && this.address)) {
          return false;
        }
        return true;
      },
    ...
    methods: {
      ...
      toGithub() {
        ...
        window.location.href = `https://github.com/login/oauth/authorize?client_id=${process.env.GITHUB_ID}`;
        // window.location.href = `https://github.com/login/oauth/authorize?client_id=895683dc144b417fdd44`;
      },
      async getToken() {
      ...
      async getFaucet() {
        const res = await faucet(this.address, this.contractAddress);
        if (res && res.status >= 0) {
